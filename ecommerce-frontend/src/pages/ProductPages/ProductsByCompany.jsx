...
//const allCategories = subcategoryData;
...
const ProductsByCompany = () => {
      ...
      if (companyId) {
        try{
          const response = await axios.get(
            `https://localhost:7061/api/Kompania/shfaqSidebarDataPerKompanine/${companyId}`
          );
  ...
  useEffect(() => {
      ...
      if (companyId) {
        try {
          const response = await axios.post(
            `https://localhost:7061/api/Kompania/shfaqProduktetSipasKompanise/${companyId}/${sortBy}/${currentPage}/${productsPerPage}`
            , filters
          );
    ...
    fetchData();
  }, [companyId, sortBy, currentPage, productsPerPage,filters,navigate]);
...
;
  ...
  const AddToCart = (idParam) => {
    const produktiNeShporte = currentCompany.products.find(
  ...
  if (companyId) {
    if (!currentCompany) {
      return <p>ERROR 404:This page doesnt exist</p>;
    }
            ...
            <div className="productContent" style={{ paddingTop: "20px" }}>
              ...
              <div className="container-fluid products-page">
                  <div className="center-text">
                    <h3 className="companyName">{currentCompany.name}</h3>
                  </div>
                  <div>
                    {filters.searchTerm !== "" && currentCompany.products.length>0   && (
                        <>
                 ...
                 <div className="product-items">
                  <div className="ProductLayout">
                    {currentCompany.products && currentCompany.products.length > 0 ? (             
                              <>
                                {currentCompany.products.map((product) => (
